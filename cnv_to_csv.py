...
data_cnv = pd.read_csv(args.raw_file, sep="\t")
columns = data_cnv.values[:, 0]
my_sample_data_column_names = ["case_id",]
for column in columns:
    my_sample_data_column_names.append(f"{column.upper()}_cnv")
my_sample_data = []
for case_id in data_cnv.columns.values[2:]:
    case_sample_data = data_cnv[case_id].values 
